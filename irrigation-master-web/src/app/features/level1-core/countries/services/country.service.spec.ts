...
const PAGINATION_URL = `${environment.apiUrl}/v1/Countries/pagination`;
...
describe('CountryService', () => {
    ...
    it('list(): sends PageNumber/PageSize and maps a successful page', () => {
        let result: ListResult<Country> | undefined;
        const countries: Country[] = [{ id: '1', name: 'España', code: 'ES', phoneCode: '+34' }];
        ...
        req.flush({ data: countries, isSuccess: true, message: 'ok', pageNumber: 1, totalPages: 1, totalCount: 1, pageSize: 50 });

        expect(result).toEqual({ isSuccess: true, message: 'ok', items: countries, totalCount: 1 });
    });
